...
def select_data(tablename): #DB 통해서, 값을 가져오는 함수
    ...
    # sql = f'SELECT * FROM {tablename} where user_id !=6 and maker_id = 1;' #6번 아이디는 제외한다.        
    cursor.execute(sql)
    # rows = list()
...
with open("/home/con/mmdetection/data/thyroid/annotations/0_tirads_classification.txt","r") as file:
...
with open(f"{save_txt_path}{year_month_day}_{times}.txt","w") as file:
    ...
    end_day = 31
    # end_day = int(year_month_day[-2:])#이미 만들어진 변수로, 당일 날짜 가져올 수 있게 처리했습니다.
    calendars[12]=list(range(start_day,end_day+1))#11월 시작일/끝일
    ###################################### 월마다 작성######################################
    ...
    # end_day = 4
    end_day = int(year_month_day[-2:])#이미 만들어진 변수로, 당일 날짜 가져올 수 있게 처리했습니다.
    calendars[1]=list(range(start_day,end_day+1))#12월 시작일/끝일
    ###################################### 월마다 작성######################################

    for month,day_list in calendars.items():
        if month <6:
            for day in day_list:
                days.append(f"2021-{month:02d}-{day:02d}")
        else:
            for day in day_list:
