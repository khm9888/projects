...
with open(f"{test_json_path}/test.json","r") as file:
...
def drawing_rec(i,v,count_values,cnt,basis=0.5,threshold=0.5):
    print()
    print(f"--{i+1}--")
    ...
    for anno in json_data["annotations"]:#어노테이션 데이터 가져옴
        if anno["image_id"]==img_id:
            ...
            if thyroid_type==1:
                ...
                print("case2")
                color = red_color
                img = cv2.rectangle(img, (values[0], values[1]), (values[0]+values[2], values[1]+values[3]), color,3 )
                box1 = ((values[0], values[1]),(values[0]+values[2], values[1]+values[3]))
                img = cv2.rectangle(img, box1[0],box1[1], color,3)
                copy_img = cv2.rectangle(copy_img, box1[0],box1[1], color,3)
                if v[1].size!=0:
...
cnt=0
v_array=value_array[0:10]
for i,v in enumerate(v_array):
    iou,cnt=drawing_rec(i,v,count_values,cnt,threshold=0.5)
    total_iou+=iou
    print(f"i:{i},total_iou:{total_iou},cnt:{cnt}")

print("-"*30)
print(f"total_iou:{total_iou}")
print(f"average_iou:{total_iou//len(v_array)}")
print("-"*30)
        
print(f"benign_count:{benign_count}")
print(f"malign_count:{malign_count}")