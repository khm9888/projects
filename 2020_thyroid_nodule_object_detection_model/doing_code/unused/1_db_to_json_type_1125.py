...
def select_data(tablename): #DB 통해서, 값을 가져오는 함수
    ...
    sql = f"SELECT * FROM {tablename} where user_id !=6 AND createdAt between '2020-11-18 00:00:00' and '2020-11-26 23:59:59';" #6번 아이디는 제외한다.        
    # SELECT * FROM THANQ WHERE user_id != 6 AND createdAt between '2020-11-18 00:00:00' and '2020-11-26 23:59:59'
    cursor.execute(sql)
    # rows = list()
...
with open("/home/con/mmdetection/data/thyroid/annotations/from_db_1118_1125.json","w") as json_file:
...
with open(f"{save_txt_path}{year_month_day}_{times}.txt","w") as file:
    ...
    calendars[11]=list(range(start_day,end_day+1))#10월 시작일/끝일
    ###################################### 월마다 작성######################################
