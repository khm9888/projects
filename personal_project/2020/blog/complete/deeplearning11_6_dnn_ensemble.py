...
x,y=split(df2,5,1)#삼성
x2,y2=split(df1,5,1)#코스피
...
x2_train,x2_test,y2_train,y2_test=tts(x2,y2,train_size=0.7)
...
dense2=Dense(30,activation="relu")(input2)
dense2=Dense(30,activation="relu")(dense2)
dense2=Dense(1,activation="relu")(dense2)

merge=Concatenate()([dense1,dense2])

output_1=Dense(10)(merge)
output_1=Dense(40)(output_1)
output_1=Dense(40)(output_1)
output_1=Dense(40)(output_1)
...
model.fit([x_train_scaled,x2_train_scaled],y_train,epochs=100,batch_size=10,validation_split=0.1,callbacks=[early])

#테스트

mse,acc=model.evaluate([x_test_scaled,x2_test_scaled],y_test,batch_size=1)

print(f"mse:{mse}")
print(f"acc:{acc}")

y_pre=model.predict([x_test_scaled,x2_test_scaled])
