...
CTP는 모든 사람이 사용할만큼 충분한 멀티탭을 가지고 있다
종류는 3구부터 8구까지 다양하게 있다
모든 사람들은 노트북만 가져오기 때문에 멀티탭 1구를 무조건 사용한다
1구를 초과해선 안 된다.
...
multitaps = tuple(map(int,input().split()))

cnt = 0
for multitap in multitaps:
    cnt+=ceil(multitap/2)
