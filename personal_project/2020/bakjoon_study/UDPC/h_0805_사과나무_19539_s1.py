...
--problem_description--
이하는 최근 사과나무 씨앗을 구매하여 농장 뒷뜰에 일렬로 $1$번부터 $N$번까지 심었다
이 나무들의 초기 높이는 모두 $0$이다.
사과나무를 무럭무럭 키우기 위해 이하는 물뿌리개 $2$개를 준비했다
한 물뿌리개는 나무 하나를 $1$만큼 성장시키고, 다른 물뿌리개는 나무 하나를 $2$만큼 성장시킨다
이 물뿌리개들은 동시에 사용해야 하며, 물뿌리개를 나무가 없는 토양에 사용할 수는 없다
두 물뿌리개를 한 나무에 사용하여 $3$만큼 키울 수도 있다.
...
두 번째 줄에는 $N$개의 정수 $h_1,h_2,\cdots,h_N$이 공백으로 구분되어 주어진다
($ 0 \leq h_i \leq 10\ 000$) $h_i$는 갊자가 바라는 $i$번째 나무의 높이이다.
...
if cnt>=cnt_3 and sum(values)%3==0:            
    print("YES")
else:
    print("NO")
