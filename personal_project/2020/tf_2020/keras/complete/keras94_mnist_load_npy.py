...
# df = pd.DataFrame(x_train,index = datasse)
...
# checkpoint = ModelCheckpoint(filepath="./model/check-{epoch:04d}-{val_loss:.4f}.hdf5",monitor="val_loss",verbose=1,save_best_only=1,save_weights_only=0)
# hist=model.fit(x_train,y_train,epochs=5,batch_size=200,validation_split=0.1,callbacks=[checkpoint])
