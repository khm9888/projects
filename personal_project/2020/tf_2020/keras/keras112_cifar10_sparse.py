...
# def build_model_cnn(optimizer=adam,node=32,loss="categorical_crossentropy"):
    ...
    conv=Conv2D(node,(2,2),padding="same",activation="relu")(input1)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu")(conv)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu")(conv)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu")(conv)
    max1=MaxPool2D(pool_size=3)(conv)

    conv=Conv2D(node*4,(2,2),padding="same",activation="relu")(max1)
    conv=Conv2D(node*4,(2,2),padding="same",activation="relu")(conv)
    max1=MaxPool2D(pool_size=3)(conv)
...
'''d:\Study\keras\keras112_cifar10.py
loss:0.9863984875679016
