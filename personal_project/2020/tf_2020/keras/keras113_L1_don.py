...
def build_model_cnn(optimizer=adam,node=32,loss="sparse_categorical_crossentropy",kernel_regularizer=l1()):
# def build_model_cnn(optimizer=adam,node=32,loss="categorical_crossentropy"):
    #2) 모델
    input1=Input(shape=(32,32,3))
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(input1)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(conv)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(conv)
    conv=Conv2D(node*2,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(conv)
    max1=MaxPool2D(pool_size=3)(conv)

    conv=Conv2D(node*4,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(max1)
    conv=Conv2D(node*4,(2,2),padding="same",activation="relu",kernel_regularizer=kernel_regularizer)(conv)
    max1=MaxPool2D(pool_size=3)(conv)
    ...
    hist=model.fit(x_train,y_train,epochs=15,batch_size=100,validation_split=0.3,callbacks=[early,modelcheck])
...
'''
Epoch 40/40
35000/35000 [==============================] - 7s 200us/step - loss: 0.1894 - acc: 0.9363 - val_loss: 7.4347 - val_acc: 0.3545
10000/10000 [==============================] - 1s 121us/step
d:\Study\keras\keras112_cifar100.py
loss:7.4105341934204105
