...
lgbm.fit(x_train,y_train,verbose=True,eval_metric=["logloss","rmse"],eval_set=[(x_train,y_train),(x_test,y_test)],early_stopping_rounds=20)
...
for idx,thresh in enumerate(thresholds):
    ...
    #2)모델구성
    selection_model = LGBMClassifier(n_estimators=100,learning_rate = 0.1,n_jobs=-1)
...
#3)훈련
selection_model.fit(selection_x_train,y_train,verbose=False, eval_metric=["logloss","rmse"],eval_set=[(selection_x_train, y_train), (selection_x_test,y_test)],early_stopping_rounds=20)
path=f"./model/lgbm_save/{__file__[-(3+17):-3]}-idx{idx_max}.dat"
