...
hite_array=hite.values
samsung_array=samsung.values
...
dense2 = LSTM(2000,activation="relu")(input2)
...
model = Model(inputs= input2, outputs = dense2)
...
#하이트, 삼성 순서로 데이터 넣음
model.fit(x_train,y_train,batch_size=1,epochs=10,validation_split=0.2,verbose=1)#,callbacks=[early])
...
samsung = samsung.reshape(1,6,1)

y_truely_pre=model.predict(samsung)
