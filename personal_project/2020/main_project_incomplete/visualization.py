...
word_sequence = " ".join(sentences).split()#str으로 쭉 나열하고, 공백으로 나눈 리스트
word_list = " ".join(sentences).split()#str으로 쭉 나열하고, 공백으로 나눈 리스트
word_list = list(set(word_list))#set을 통해서 중복값을 없애고, 리스트로 변환
...
for i in range(1, len(word_sequence) - 1):#첫번째 단어와, 마지막 단어에 관해서는 학습 안 한다.
