        ...
        if anno["image_id"]==img_id:
            ...
            if thyroid_type==1:#benign
                ...
                if predict==0:# Zero Benign
                    ...
                    dict_nodules[nodule][2][0]+=1
                    dict_nodule_maker_ids[maker_id][nodule][2][0]+=1
                    write_path = ob_path
                    
                elif predict==1:#True Benign
                    count_values[0][thyroid_type-1]+=1
                    dict_makers[maker_id][0][0]+=1
                    dict_nodules[nodule][0][0]+=1
                    dict_nodule_maker_ids[maker_id][nodule][0][0]+=1
                    write_path = tb_path
                    
                elif predict==2:#False Malign
                    count_values[1][thyroid_type-1]+=1
                    dict_makers[maker_id][1][0]+=1
                    dict_nodules[nodule][1][0]+=1
                    dict_nodule_maker_ids[maker_id][nodule][1][0]+=1
                    write_path = fm_path
                

   
                if maker_id==1:
                    maker_name ="1_삼성RS80A"
                elif maker_id==2:
                    maker_name ="2_삼성RS85"
                elif maker_id==3:
                    maker_name ="3_알피니언diamond"
                elif maker_id==4:
                    maker_name ="4_알피니언platinum"
                elif maker_id==5:
                    maker_name ="5_삼성60"
                elif maker_id==6:
                    maker_name ="6_필립스"
                ...
                if nodule==1:
                    ...
                    nodule_name ="5_cat2_양성"
                elif nodule==6:
                    nodule_name ="6_cat5_양성의심"
                elif nodule==7:
                    nodule_name ="7_cat6_악성"
                elif nodule==8:
                    nodule_name ="8_c_benign"
                elif nodule==9:
                    nodule_name ="9_c_malign"
                else:
                    nodule_name ="0"
                      
                cv2.imwrite(f"{write_path}{name}_{maker_name}_{nodule_name}.{format0}",img)#id까지 표시했습니다.
            # print("predict_list")           
            # print(predict_list)           
            ...
            if thyroid_type==2:#malign
                ...
                if predict==0:#Zero malign
                    ...
                    dict_nodules[nodule][2][1]+=1
                    dict_nodule_maker_ids[maker_id][nodule][2][1]+=1
                    write_path = om_path
                elif predict==1:#False Benign
                    count_values[0][thyroid_type-1]+=1
                    dict_makers[maker_id][0][1]+=1
                    dict_nodules[nodule][0][1]+=1
                    dict_nodule_maker_ids[maker_id][nodule][0][1]+=1
                    write_path = fb_path
                elif predict==2:#True malign
                    count_values[1][thyroid_type-1]+=1  
                    dict_makers[maker_id][1][1]+=1
                    dict_nodules[nodule][1][1]+=1
                    dict_nodule_maker_ids[maker_id][nodule][1][1]+=1
                    write_path = tm_path
...
for i,v in enumerate(test_result[:]): #모든 예측값에 대해서 가져온다.
    ...
    count_values=drawing_rec(i,v,count_values,threshold=0.3,basis=0.2)
...
TB,TM,FM,FB,OB,OM = 0,0,0,0,0,0

record_dataset_num = input("데이터셋 번호")
record_epoch = input("epoch 번호")


with open(f"/mnt/sdb/AI/work/result_txt/{year_month_day}_{record_dataset_num}.txt","a") as file:
    print()
    file.write("\n")
    print(f"#{record_dataset_num}_{record_epoch}")
    file.write(f"#{record_dataset_num}_{record_epoch}\n\n")
    print()
    print(date)
    file.write(f"{date}\n")
    ...
    for i in range(10):
        print(f"{i}번째_{list_nodule_names[i]:17s} : {dict_nodules[i]},{add(dict_nodules[i])}")
        file.write(f"{i}번째_{list_nodule_names[i]:17s} : {dict_nodules[i]},{add(dict_nodules[i])}\n")

# 이 부분은 9,4,3 제품군 제외
    print()
    ment="0~9번의 제품군 중 3,4,9번을 제외하고 3,4번 결절을 제외한 결과입니다."
    print(ment)
    file.write("\n")
    file.write(ment+"\n")
    except_maker_ids = [3,4,9]
    except_nodules = [2,3,8,9]
    # except_nodules = []
    for i in range(10):
        if i not in except_maker_ids:
            for j in range(10):
                if j not in except_nodules:
                # print("dict_nodule_maker_ids[i][j]")
                # print(dict_nodule_maker_ids[i][j])
                    tb,tm = dict_nodule_maker_ids[i][j][0][0],dict_nodule_maker_ids[i][j][1][1]
                    fm,fb = dict_nodule_maker_ids[i][j][1][0],dict_nodule_maker_ids[i][j][0][1]
                    ob,om = dict_nodule_maker_ids[i][j][2][0],dict_nodule_maker_ids[i][j][2][1]
                    TB +=tb
                    TM +=tm
                    FM +=fm
                    FB +=fb
    ...
    if TM!=0:
        sensitivity = TM/(TM+FB+OM)
    else:
        sensitivity=0
    if TB!=0:
        specificity = TB/(TB+FM+OB)
    else:
        specificity=0
    accuracy = (TB+TM)/(TM+FB+OM+TB+FM+OB)

    # print(f"이미지 총 개수는 {len(test_result)}")
    print(f"어노테이션 총 개수는 {TB+TM+FB+FM+OB+OM}")

    print(f"TB:{TB},TM:{TM},FM:{FM},FB:{FB},OB:{OB},OM:{OM}")
    ...
    file.write(f"TB:{TB},TM:{TM},FM:{FM},FB:{FB},OB:{OB},OM:{OM}\n")
...
TB,TM,FM,FB,OB,OM = count_values[0][0],count_values[1][1],count_values[1][0],count_values[0][1],count_values[2][0],count_values[2][1]
if TM!=0:
    sensitivity = TM/(TM+FB+OM)
else:
    sensitivity=0
...
#이 부분은 모든 제품군 포함
    ...
    for i in range(10):
        ...
        fm,fb = dict_makers[i][1][0],dict_makers[i][0][1]
    ...
    print("-"*20+"결절별 값 정리"+"-"*20)
    file.write("-"*20+"결절별 값 정리"+"-"*20+"\n")
    for i in range(1,10):
        tb,tm = dict_nodules[i][0][0],dict_nodules[i][1][1]
        fm,fb = dict_nodules[i][1][0],dict_nodules[i][0][1]
        ob,om = dict_nodules[i][2][0],dict_nodules[i][2][1]
        
        total = add(dict_nodules[i])
        total_b = tb+fm+ob
        total_m = tm+fb+om
        print(f"tm:{tm}({total_m}개),tb:{tb}({total_b}개),total:{total}")
        file.write(f"tm:{tm}({total_m}개),tb:{tb}({total_b}개),total:{total}\n")
        ...
        print(f"{i}번째_{list_nodule_names[i]:17s} : sensitivity : {(tm)/(total_m)*100:.1f}%, specificity : {(tb)/(total_b)*100:.1f}%, accuracy : {(tm+tb)/total*100:.1f}%")
        file.write(f"{i}번째_{list_nodule_names[i]:17s} : sensitivity : {(tm)/(total_m)*100:.1f}%, specificity : {(tb)/(total_b)*100:.1f}%, accuracy : {(tm+tb)/total*100:.1f}%\n")
        print()
        file.write("\n")
...
with open(test_result_image_path+"result.txt","a") as file:
    file.write("\n")
    file.write(f"{record_dataset_num}번\n")
    file.write(f"TB:{TB},TM:{TM},FM:{FM},FB:{FB},OB:{OB},OM:{OM}\n")
    file.write("\n")
    
with open(f"{test_result_image_path}/result.json","r") as file:#의사분들이 체크해준 값
    contioust_json_data = json.load(file)


  
for i,ii in contioust_json_data.items():
    # print("type(i)")
    # print(type(i))
    for j,jj in ii.items():
        for k,kk in enumerate(jj):
            for m,mm in enumerate(kk):
    

                contioust_json_data[i][j][k][m]+=dict_nodule_maker_ids[int(i)][int(j)][k][m]
                
with open(f"{test_result_image_path}/result.json","w") as file:#의사분들이 체크해준 값
    json.dump(contioust_json_data,file)