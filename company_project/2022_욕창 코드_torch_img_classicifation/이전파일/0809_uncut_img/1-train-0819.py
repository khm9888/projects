...
def mktrainval(args, logger,i):
    ...
    train_set = cus_dataset.CustomDataset(args.datadir, args.imagedir, set_name=f'train_{i}', cls_num=6, transform=train_tx)
    valid_set = cus_dataset.CustomDataset(args.datadir, args.imagedir, set_name=f'val_{i}', cls_num=6, transform=val_tx)     
...
def main(args):
    ...
    # load dataset
    train_set, valid_set, train_loader, valid_loader = mktrainval(args, logger,0)
    ...
    bestname = pjoin(saveroot, f'best.pth')
    bestname_num = pjoin(saveroot, f'best-{args.data_num}.pth')
    savename_1=f"{bestname_num.rsplit('.')[0]}-{today}.{bestname_num.rsplit('.')[1]}"
    lastname = pjoin(saveroot, f'last.pth')
    lastname_num = pjoin(saveroot, f'last-{args.data_num}.pth')
    savename_2=f"{lastname_num.rsplit('.')[0]}-{today}.{lastname_num.rsplit('.')[1]}"
...
####################### train ###############################################
        ...
        for x, y in recycle(train_loader): # ??
            ...
            if mixup > 0.0: # 0.0 if data < 20_000
                x, y_a, y_b = mixup_data(x, y, mixup_l) # 데이터 랜덤하게 쓰는 듯 ?

            # Compute output
            with chrono.measure('fprop'): 
                logits = model(x)
                if mixup > 0.0:
                    c = mixup_criterion(cri, logits, y_a, y_b, mixup_l) # 랜덤하게 쓴 걸로 성능 측정 
                else:
                    c = cri(logits, y)
            ...
            with chrono.measure('grads'):
                (c / args.batch_split).backward() # ... 그냥 가중치 낮춰서 백워드 하는 건가
                accum_steps += 1
...
####################### train ###############################################
        ...
        torch.save(model, lastname_num)
        torch.save(model, savename_2)
        # save best 
        cur_metric = np.mean(all_c)
        if cur_metric <= min(val_metric):
            torch.save(model, bestname)
            torch.save(model, bestname_num)
            torch.save(model, savename_1)
