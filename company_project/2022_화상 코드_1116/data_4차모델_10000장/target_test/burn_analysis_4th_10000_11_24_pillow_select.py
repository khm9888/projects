...
def result():
    ...
    os.makedirs("{path_burn_image}/record/img_{today[2:]}_1",exist_ok=True)
    save_path = f"{path_burn_image}/record/img_{today[2:]}_1/"



    def pil_draw_rect(image, point1, point2,true,predict,rate):
        ...
        if true == predict:
            ...
            outline = (255, 0, 0)#rgb
        draw.rectangle((point1, point2), outline=outline, width=3)
        
        if true == predict:
            draw.text((x1,(y1+y2)//2),f"true : {true[:5]}")
        else:
            draw.text((x1,(y1+y2)//2),f"true : {true[:5]}, predict :{predict[:5]}")
        # draw.text((x1,(y1+y2)//2),f"true : {true}, predict :{predict}, rate: {rate:.3f}")
    ...
    for i in tqdm(range(len(df_plus_convert))):
        ...
        image1 =Image.open(load_path+origin_file_name)
        ...
        convert_image.save(f"{save_path}{cls_name}/{predict}/{origin_file_name}")
        convert_image.save(f"{save_path}/{origin_file_name}")
        # input()
