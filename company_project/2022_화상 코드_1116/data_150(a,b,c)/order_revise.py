...
path_burn_image = "/home/lab/burn/data/data/con_test_211112/"
...
def step_3(part):#변환된 엑셀 파일을 통해 각 raw 별 값을 폴더에 집어 넣으려고 한다.
    ...
    save_dict = dict()
    save_dict["origin_file_name"]=list()
    ...
    for raw_num in range(len(df_part_data)):
        ...
        cls = (df_part_data.iloc[raw_num]["class"])#a-1830.JPG
        print(file_name)
        print(cls)
        ...
        # input()
        croppedImage=read_image[y1:y2,x1:x2]
        ...
        print(rename_file)
        cv2.imwrite(save_img_loc+rename_file,croppedImage)
        # input()
        # cv2.destroyAllWindow()
        
        
        save_dict["origin_file_name"].append(file_name)
        save_dict["file_name"].append(rename_file)
        save_dict["class"].append(class_name)
        save_dict["x1"].append(x1)
        save_dict["x2"].append(x2)
        save_dict["y1"].append(y1)
        save_dict["y2"].append(y2)
    
    crop_data_df=pd.DataFrame(save_dict)
    crop_data_df=crop_data_df[(crop_data_df["class"]!="1st")]

    xlsx_name = part.split(".")[0]
    crop_data_df.to_excel(f"{path_burn_image}{xlsx_name}_test.xlsx",index=0)
    crop_data_df.to_csv(f"{path_burn_image}{xlsx_name}_test.csv",index=0)
    crop_data_df.to_csv(f"{path_burn_image}test.csv",index=0)
...
step_3("part_b_trans.xlsx")
step_3("part_c_trans.xlsx")
