...
def main():
    ...
    root_dir = f'{path_burn_image}'
    # ["1st",'above_3rd', 'deep_2nd', 'normal', 'not_sure', 'superficial_2nd']
    ...
    choose_abc = "/c/"
    data_path = f'{path_burn_image}image_crop{choose_abc}' # test_210914_crop/'
    ...
    if path_only:
        ...
        p1 = root_dir
        p2 = os.path.join(root_dir, f'../image_after{choose_abc}')
    ...
    value_list = ['above_3rd', 'deep_2nd', 'normal', 'not_sure', 'superficial_2nd']
    # trans_values = dict()
    # for i,value in enumerate(value_list):
    #     trans_values[i]=value
        
    pred_list_trans = list()
    y_list_trans = list()
    for i,pred in enumerate(pred_list):
        pred_list_trans.append(value_list[pred_list[i]])
        y_list_trans.append(value_list[y_list[i]])
    prob_list_trans = list(map(max,prob_list))    
        
    df_result=pd.DataFrame({"file_name_r":file_list,"predict":pred_list_trans,"test":y_list_trans,"prob":prob_list_trans})
    df_result.to_csv(f"{path_burn_image}/result_bit_{choose_abc[1]}.csv",index=0)
